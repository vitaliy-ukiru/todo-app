...
type Querier interface {
	...
	PingTaskDB(ctx context.Context) (pgconn.CommandTag, error)
	// PingTaskDBBatch enqueues a PingTaskDB query into batch to be executed
	// later by the batch.
	PingTaskDBBatch(batch genericBatch)
	// PingTaskDBScan scans the result of an executed PingTaskDBBatch query.
	PingTaskDBScan(results pgx.BatchResults) (pgconn.CommandTag, error)
}
...
func PrepareAllQueries(ctx context.Context, p preparer) error {
	...
	if _, err := p.Prepare(ctx, pingTaskDBSQL, pingTaskDBSQL); err != nil {
		return fmt.Errorf("prepare query 'PingTaskDB': %w", err)
	}
	return nil
}
...
const pingTaskDBSQL = `SELECT 1;`

// PingTaskDB implements Querier.PingTaskDB.
func (q *DBQuerier) PingTaskDB(ctx context.Context) (pgconn.CommandTag, error) {
	ctx = context.WithValue(ctx, "pggen_query_name", "PingTaskDB")
	cmdTag, err := q.conn.Exec(ctx, pingTaskDBSQL)
	if err != nil {
		return cmdTag, fmt.Errorf("exec query PingTaskDB: %w", err)
	}
	return cmdTag, err
}

// PingTaskDBBatch implements Querier.PingTaskDBBatch.
func (q *DBQuerier) PingTaskDBBatch(batch genericBatch) {
	batch.Queue(pingTaskDBSQL)
}

// PingTaskDBScan implements Querier.PingTaskDBScan.
func (q *DBQuerier) PingTaskDBScan(results pgx.BatchResults) (pgconn.CommandTag, error) {
	cmdTag, err := results.Exec()
	if err != nil {
		return cmdTag, fmt.Errorf("exec PingTaskDBBatch: %w", err)
	}
	return cmdTag, err
}
