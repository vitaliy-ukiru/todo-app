...
type Querier interface {
	...
	PingUserDB(ctx context.Context) (pgconn.CommandTag, error)
	// PingUserDBBatch enqueues a PingUserDB query into batch to be executed
	// later by the batch.
	PingUserDBBatch(batch genericBatch)
	// PingUserDBScan scans the result of an executed PingUserDBBatch query.
	PingUserDBScan(results pgx.BatchResults) (pgconn.CommandTag, error)
}
...
func PrepareAllQueries(ctx context.Context, p preparer) error {
	...
	if _, err := p.Prepare(ctx, pingUserDBSQL, pingUserDBSQL); err != nil {
		return fmt.Errorf("prepare query 'PingUserDB': %w", err)
	}
	return nil
}
...
const findUserByIDSQL = `SELECT id, email, username, password, created_at
FROM users
WHERE id = $1::uuid;`
...
const pingUserDBSQL = `SELECT 1;`

// PingUserDB implements Querier.PingUserDB.
func (q *DBQuerier) PingUserDB(ctx context.Context) (pgconn.CommandTag, error) {
	ctx = context.WithValue(ctx, "pggen_query_name", "PingUserDB")
	cmdTag, err := q.conn.Exec(ctx, pingUserDBSQL)
	if err != nil {
		return cmdTag, fmt.Errorf("exec query PingUserDB: %w", err)
	}
	return cmdTag, err
}

// PingUserDBBatch implements Querier.PingUserDBBatch.
func (q *DBQuerier) PingUserDBBatch(batch genericBatch) {
	batch.Queue(pingUserDBSQL)
}

// PingUserDBScan implements Querier.PingUserDBScan.
func (q *DBQuerier) PingUserDBScan(results pgx.BatchResults) (pgconn.CommandTag, error) {
	cmdTag, err := results.Exec()
	if err != nil {
		return cmdTag, fmt.Errorf("exec PingUserDBBatch: %w", err)
	}
	return cmdTag, err
}
